//! Native reconstruction-prime P ring: prove the CRT recombination of the
//! three RNS channel residues inside the 104-bit P field ring.
//!
//! The reconstructed value is written in Garner mixed-radix form
//! `v = r0 + q0 * t1 + q0 * q1 * t2` with `r0 < q0`, `t1 < q1`, `t2 < q2`.
//! Every term is below Q < P, so the relation holds over the integers and
//! is one linear R1CS row in the P ring. The residues and Garner digits are
//! the committed witness; the reconstructed value is the public input.
//!
//! Run with:
...
use latticefold::{
    ...
    vdkg_params::{N4096_THRESHOLD_MODULI, N4096_THRESHOLD_MODULUS_PRODUCT},
};
...
fn reconstruction_r1cs() -> R1CS<PRing> {
    let [q0, q1, _] = N4096_THRESHOLD_MODULI;
...
/// Garner digits of a Z_Q value with respect to the q0/q1/q2 chain.
fn garner_digits(value: u128) -> (u64, u64, u64) {
    ...
    assert!(t2 < q2, "value is not in the Z_Q reconstruction range");
    (r0, t1, t2)
}

fn main() -> Result<(), Box<dyn std::error::Error>> {
    // A representative aggregated Z_Q value produced by threshold
    // reconstruction on the three channels.
    let value: u128 = N4096_THRESHOLD_MODULUS_PRODUCT - 0xDEAD_BEEF_CAFE;
    let (r0, t1, t2) = garner_digits(value);
    assert_eq!(
        r0 as u128
            + N4096_THRESHOLD_MODULI[0] as u128 * t1 as u128
            + N4096_THRESHOLD_MODULI[0] as u128 * N4096_THRESHOLD_MODULI[1] as u128 * t2 as u128,
        value
    );
    ...
    println!("v = r0 + q0*t1 + q0*q1*t2 over the 104-bit P ring: validated");
